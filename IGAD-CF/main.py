...
def arg_parse():
    ...
    parser.add_argument('--hyperparameter', dest='beta', default=0.6, help='hyperparameter')
...
if __name__ == '__main__':
    ...
    if args.DS in ["HSE", "MMP", "p53", "PPAR-gamma"]:
        DS1 = f"Tox21_{args.DS}_training"
        DS2 = f"Tox21_{args.DS}_testing"
        graphs1 = load_data.read_graphfile(f"./dataset", DS1, max_nodes=0)
        graphs2 = load_data.read_graphfile(f"./dataset", DS2, max_nodes=0)
        graphs = graphs1 + graphs2
    elif args.DS == "hERG":
        setup_seed(1)
        data = Tox(name=args.DS)
        ...
        for idx, graph in enumerate(graphs):
            ...
            if graph_label[idx] == 0:
                graphs_label.append(0)
            else:
                graphs_label.append(1)


    else:
        graphs = load_data.read_graphfile(f"./dataset", args.DS, max_nodes=0)
    datanum = len(graphs)
    max_nodes_num = max([G.number_of_nodes() for G in graphs])
    ...
    for i, igraph in enumerate(graphs):
        try:
            adj_matrix_list.append(nx.adjacency_matrix(graphs[i]).toarray())
            graphs_label.append(igraph.graph['label'])
        except Exception as e:
            print(e)
        finally:
            continue

    max_nodes = max([arr.shape[0] for arr in adj_matrix_list])
    ndarray_3d = np.zeros((len(adj_matrix_list), max_nodes, max_nodes))

    for i, arr in enumerate(adj_matrix_list):
        n = arr.shape[0]
        ndarray_3d[i, :n, :n] = arr


    adj_matrix = np.stack(ndarray_3d, axis=0)
    temp_list = []

    for i in graphs_label:
        if i == 0:
            temp_list.append(0)
        else:
            temp_list.append(1)
    graphs_label = temp_list
    ...
    if os.path.exists(f"./data/" + args.DS + args.node_features + 'trainCGC_list.pkl'):
        print("loading...")
        CGC_list = load_generated_data(args.DS + args.node_features, 'trainCGC_list.pkl')

    else:
        CGC_list = generation(args_GCG, graphs_num, nodes_num_list, attrs_dim, loaded_datas)
        save_generated_data(args.DS + args.node_features, 'trainCGC_list.pkl', CGC_list)
